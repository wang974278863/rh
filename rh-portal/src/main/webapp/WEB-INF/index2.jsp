...
<head>
	...
	<style type="text/css">
		...
		.life .listimg{ float:right;  height:auto; overflow:hidden;}
		.life .listimg li{ float:left; width:187px; height:150px; padding-bottom:10px; }
	    .life .listimg .pr0{ padding-right:0;}
		.life .listimg li a{ display:block; height:140px; padding-top:6px; background:#f2f2f2; text-align:center; line-height:18px; }
		.life .listimg li a img{ display:block; margin:0 auto;}
		.life .listimg div{ height:30px; clear:both; padding-left:10px;}
		.life .listimg div a{ color:#0033FF;}
	</style>
</head>
			...
			<div>
				...
				<ul class="list-unstyled list-new">
					<c:forEach items="${xyzx }" var="obj">
						<li>
							<a href="/zszx/view/${obj.id }">${obj.title }</a><a href="/zszx/view/${obj.id }" class="public-detail">查看详情&gt;&gt;</a>
						</li>
					</c:forEach>
				</ul>
			</div>
							...
							<c:forEach items="${xyfc }" var="obj" varStatus="status">
								<li class="<c:if test="${((status.index + 1) % 4) eq 0 }">pr0</c:if>">
									<a href="/xyfc/${obj.id }" title="${obj.title }" target="_blank">
										<img class="lazy" src="${obj.pic }" width="180" height="115" style="display: block;">
										<p>${obj.title }</p>
									</a>
								</li>
							</c:forEach>
...
<div id="guestBook" class="zxly_from" style="display:none;background-color:#B2DFEE; width: 780px;height: auto;overflow: hidden;padding-top: 30px;padding-left: 15px;">
			  ...
			  <input maxlength="80" name="contactWay" size="57" class="ipt-txt" type="text">
			        ...
			        <div style="float: left;width: 100%;margin: 10px 90px">
			              <div class="d1" >留言内容：<font color="#FF0000">*</font>
			      		  ...
			      		  <textarea maxlength="120" placeholder="最多输入120个字" name="content" cols="55" rows="3" style="margin-left: 75px;"></textarea>
			        </div><br>
